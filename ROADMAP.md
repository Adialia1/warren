...
| Forge | `Forge` — repo refs, git auth, PR open/find, checks, error taxonomy | `next` — paid by App installation tokens via the manifest flow. Design doc first. |
| Issue tracker | `IssueTracker` — capability-flagged (`supportsPlans`) | `next`, after the Forge campaign — paid by Linear. |
...
1. **The Forge campaign: `Forge` paid by GitHub App installation tokens.** Descoped from the old GitHub App campaign on 2026-08-03 — the login half moved to Deferred until paid (see Decisions). One design doc comes first, modeled on `docs/design/runtime-provider-contract.md`, written while the Now batch runs. Implementation starts on an explicit owner go/no-go. Internal order: consolidate the three GitHub REST clients into `src/forge/github/http.ts` first. Then cut a capability-minimal `Forge` contract, with a FakeForge acceptance provider as implementation #2. Then the credential story, once: the forge mints and refreshes its own tokens. Two mechanisms back it — a fine-grained PAT, and installation tokens from a GitHub App that each deployment registers through the manifest flow. PAT mode is a permanent peer, never a legacy path. Hourly-expiring installation tokens are the forcing function. A static token threaded through ~30 files cannot express them, so the App pays for the consolidation. Falsification tests: a FakeForge project completes dispatch → reap → push → PR with zero domain-code changes, and scenario 39 stays green at every commit.
...
6. **The flagship extension — prove the Tier-1 packaging story with one real consumer.** The bus is live. The missing parts are the loader, the package format, and the distribution channel. Today a new consumer requires an edit to the compiled-in array in `src/server/main/lifecycle-bus-wiring.ts` and a rebuild. Do not design that mechanism on speculation. Build one first-party extension out-of-core: the audit log or a Slack notification sink. Package, version, and load it exactly as a third party's — a container image plus the `PLUGIN_*` env contract from the PHILOSOPHY Tier-1 sketch. The friction that appears during the build becomes the spec for the manifest, the loader config, and the payload JSON-schema story. Depends only on the live bus, so it can run parallel with items 3–5.
...
- **Audit log** (old R-16) — **Tier 1**. The flagship event-bus consumer, and a candidate for Next item 6. A core `audit_log` table would make the same data twice. **Amended 2026-08-03:** no longer waits on `Actor.subject` — actor kind (operator, agent, spectator) is enough attribution until named tokens land.
